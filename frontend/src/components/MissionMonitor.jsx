...
import { Play, Pause, Square, Clock, Target, CheckCircle, XCircle, Loader2 } from "lucide-react"
...
const MissionMonitor = () => {
  ...
  const mockMission = {
    ...
    waypoints: [
      ...
      { lat: 37.7755, lng: -122.418, name: "Checkpoint 1" },
      { lat: 37.776, lng: -122.4165, name: "Checkpoint 2" },
      { lat: 37.7765, lng: -122.415, name: "Checkpoint 3" },
      { lat: 37.777, lng: -122.4135, name: "End Point" },
    ],
    currentPosition: { lat: 37.776, lng: -122.4165 },
    drone: {
      id: "drone-001",
      name: "DJI Mavic Pro",
      battery: 78,
  ...
  const cleanupMap = useCallback(() => {
    ...
    if (currentPositionMarkerRef.current && currentPositionMarkerRef.current.setMap) {
      currentPositionMarkerRef.current.setMap(null)
      currentPositionMarkerRef.current = null
    }
  }, [])

  // Load mission data - runs once on mount
  useEffect(() => {
    let isMounted = true
    ...
    const loadMission = async () => {
      try {
        ...
        if (isMounted) {
          ...
          setIsLoading(false) // Add this line to stop loading regardless of map
        }
      } catch (error) {
  ...
  // Initialize Google Maps - runs once when mission is loaded
  useEffect(() => {
    if (!mission || !mapRef.current || isMapInitialized) {
      return
    }
    ...
    const initMap = async () => {
      ...
      if (!apiKey || apiKey === "YOUR_GOOGLE_MAPS_API_KEY" || apiKey === "undefined") {
        ...
        return // Remove setIsLoading(false) from here
      }
  ...
  }, [mission, isMapInitialized])

  // Initialize markers and flight path - runs when map and flightPath are ready
  useEffect(() => {
    if (!map || !flightPath.length || !window.google) {
      return
    }
  ...
  }, [missionStatus, flightPath, progress])
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="bg-white border-b border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Mission Monitor</h1>
            <p className="text-gray-600 mt-1">{mission?.name || "Loading mission..."}</p>
            {!map && (
              <div className="mt-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                <p className="text-sm text-yellow-800">
                  <strong>Note:</strong> Google Maps API key not configured. This is a demonstration interface with mock
                  data.
                </p>
              </div>
            )}
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-gray-600">{formatTime(estimatedTimeRemaining)} remaining</span>
            </div>
          </div>
        </div>
      </div>
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            top: `${y}px`,
                            backgroundColor:
                              index === 0 ? "#4ade80" : index === flightPath.length - 1 ? "#ef4444" : "#3b82f6",
                          }}
                        >
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-medium">{mission?.startTime?.toLocaleTimeString()}</span>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-medium">
                  {currentPosition ? `${currentPosition.lat.toFixed(4)}, ${currentPosition.lng.toFixed(4)}` : "N/A"}
                </span>
              </div>
            </div>
          </div>
                ...
                <div key={index} className="flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-50">
                  <div
                    className={`w-3 h-3 rounded-full ${
                      index === 0 ? "bg-green-500" : index === flightPath.length - 1 ? "bg-red-500" : "bg-blue-500"
                    }`}
                  />
